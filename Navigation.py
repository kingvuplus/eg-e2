...
class Navigation:
    ...
    def wakeupCheck(self):
        ...
        if abs(timediff_wakeup) <= 600 or abs(timediff_timer) <= 600:
            ...
            if self.timertime > 0:
                print "[NAVIGATION] no timers in the time window, next '%s' starts at %s" % ({0: 'record-timer',
                  1: 'zap-timer',
                  2: 'power-timer',
                  3: 'plugin-timer'}[self.wakeuptyp], ctime(self.timertime))
    ...
    def TimeSynctimer(self):
        ...
        self.syncCount += 1
        if now <= 31536000:
            if self.syncCount <= 24 and now <= 31536000:
                self.timesynctimer.start(5000, True)
            else:
                print '~' * 100
                            ...
                            if type_service == 61166:
                                ...
                                if config.usage.frontend_priority_dvbc.value != '-2':
                                    if config.usage.frontend_priority_dvbc.value != config.usage.frontend_priority.value:
                                        ...
                                        setPriorityFrontend = True
                            elif config.usage.frontend_priority_dvbs.value != '-2':
