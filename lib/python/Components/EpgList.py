# Embedded file name: /usr/lib/enigma2/python/Components/EpgList.py
...
class EPGList(HTMLComponent, GUIComponent):
    ...
    def __init__(self, type = EPG_TYPE_SINGLE, selChangedCB = None, timer = None, time_epoch = 120, overjump_empty = False, graphic = False):
        ...
        self.borderColorService = 12632256
        self.foreColor = 16777215
        self.foreColorSelected = 16777215
        self.backColor = 2966878
        self.backColorSelected = 14063104
        self.foreColorService = 16777215
        self.backColorService = 2966878
        self.foreColorNow = 16777215
        self.foreColorNowSelected = 16777215
        self.backColorNow = 33375
        self.backColorNowSelected = 14063104
        self.foreColorServiceNow = 16777215
        self.backColorServiceNow = 33375
        self.foreColorRecord = 16777215
        self.backColorRecord = 13710131
        self.foreColorRecordSelected = 16777215
        self.backColorRecordSelected = 10364454
        self.foreColorZap = 16777215
        self.backColorZap = 6722662
        self.foreColorZapSelected = 16777215
        self.backColorZapSelected = 4415811
        ...
        if self.type == EPG_TYPE_GRAPH or self.type == EPG_TYPE_INFOBARGRAPH:
            if self.type == EPG_TYPE_GRAPH:
                ...
                if config.epgselection.graph_heightswitch.value:
                    ...
                    elif self.listHeight / config.epgselection.graph_itemsperpage.value * 3 <= 45:
    ...
    def buildSingleEntry(self, service, eventId, beginTime, duration, EventName):
        ...
        res = [None, (eListboxPythonMultiContent.TYPE_TEXT,
          ...
          _(strftime('%a', t))), (eListboxPythonMultiContent.TYPE_TEXT,
          r2.x,
          r2.y,
          r2.w,
          r1.h,
            ...
            if self.wasEntryAutoTimer and clock_types in (2, 7, 12):
                if self.screenwidth and self.screenwidth == 1920:
                    res.extend(((eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND,
                      ...
                      25,
                      25,
                      self.clocks[clock_types]), (eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND,
                      r3.x + r3.w - 52,
                      r3.h / 2 - 13,
                      25,
                      25,
                      self.autotimericon), (eListboxPythonMultiContent.TYPE_TEXT,
                      r3.x,
                      r3.y,
                      r3.w - 52,
                      r3.h,
                ...
                res.extend(((eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND,
                  ...
                  25,
                  25,
                  self.clocks[clock_types]), (eListboxPythonMultiContent.TYPE_TEXT,
                  r3.x,
                  r3.y,
                  r3.w - 25,
                  r3.h,
                ...
                if self.wasEntryAutoTimer and clock_types in (2, 7, 12):
                    if self.screenwidth and self.screenwidth == 1920:
                        res.extend(((eListboxPythonMultiContent.TYPE_TEXT,
                          ...
                          1,
                          RT_HALIGN_LEFT | RT_VALIGN_CENTER,
                          EventName), (eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND,
                          pos,
                          r3.h / 2 - 13,
                          25,
                          25,
    ...
    def buildGraphEntry(self, service, service_name, events, picon):
        ...
        if bgpng is not None and self.graphic:
            ...
            res.append(MultiContentEntryPixmapAlphaBlend(pos=(r1.x + self.serviceBorderWidth, r1.y + self.serviceBorderWidth), size=(r1.w - 2 * self.serviceBorderWidth, r1.h - 2 * self.serviceBorderWidth), png=bgpng, flags=BT_SCALE))
        else:
            res.append(MultiContentEntryText(pos=(r1.x, r1.y), size=(r1.w, r1.h), font=0, flags=RT_HALIGN_LEFT | RT_VALIGN_CENTER, text='', color=serviceForeColor, color_sel=serviceForeColor, backcolor=serviceBackColor, backcolor_sel=serviceBackColor, border_width=self.serviceBorderWidth, border_color=self.borderColorService))
        displayPicon = None
        if self.showPicon:
            if picon is None:
                picon = getPiconName(service)
                curIdx = self.l.getCurrentSelectionIndex()
                self.list[curIdx] = (service,
                 service_name,
                 events,
                 picon)
            ...
            if displayPicon is not None:
                res.append(MultiContentEntryPixmapAlphaBlend(pos=(r1.x + self.serviceBorderWidth, r1.y + self.serviceBorderWidth), size=(piconWidth, piconHeight), png=displayPicon, backcolor=None, backcolor_sel=None, flags=BT_SCALE | BT_KEEP_ASPECT_RATIO))
        ...
        if self.showServiceTitle or displayPicon is None:
            res.append(MultiContentEntryText(pos=(r1.x + piconWidth + self.serviceBorderWidth + self.serviceNamePadding, r1.y + self.serviceBorderWidth), size=(namewidth - 2 * (self.serviceBorderWidth + self.serviceNamePadding), r1.h - 2 * self.serviceBorderWidth), font=namefont, flags=namefontflag, text=service_name, color=serviceForeColor, color_sel=serviceForeColor, backcolor=serviceBackColor, backcolor_sel=serviceBackColor))
        if self.borderTopPix is not None and self.graphic:
            res.append(MultiContentEntryPixmapAlphaTest(pos=(r1.x, r1.y), size=(r1.w, self.serviceBorderWidth), png=self.borderTopPix, flags=BT_SCALE))
            res.append(MultiContentEntryPixmapAlphaTest(pos=(r2.x, r2.y), size=(r2.w, self.eventBorderWidth), png=self.borderTopPix, flags=BT_SCALE))
        if self.borderBottomPix is not None and self.graphic:
            res.append(MultiContentEntryPixmapAlphaTest(pos=(r1.x, r1.h - self.serviceBorderWidth), size=(r1.w, self.serviceBorderWidth), png=self.borderBottomPix, flags=BT_SCALE))
            res.append(MultiContentEntryPixmapAlphaTest(pos=(r2.x, r2.h - self.eventBorderWidth), size=(r2.w, self.eventBorderWidth), png=self.borderBottomPix, flags=BT_SCALE))
        if self.borderLeftPix is not None and self.graphic:
            res.append(MultiContentEntryPixmapAlphaTest(pos=(r1.x, r1.y), size=(self.serviceBorderWidth, r1.h), png=self.borderLeftPix, flags=BT_SCALE))
            res.append(MultiContentEntryPixmapAlphaTest(pos=(r2.x, r2.y), size=(self.eventBorderWidth, r2.h), png=self.borderLeftPix, flags=BT_SCALE))
        if self.borderRightPix is not None and self.graphic:
            res.append(MultiContentEntryPixmapAlphaTest(pos=(r1.w - self.serviceBorderWidth, r1.x), size=(self.serviceBorderWidth, r1.h), png=self.borderRightPix, flags=BT_SCALE))
            res.append(MultiContentEntryPixmapAlphaTest(pos=(r2.x + r2.w - self.eventBorderWidth, r2.y), size=(self.eventBorderWidth, r2.h), png=self.borderRightPix, flags=BT_SCALE))
        if self.graphic:
            if not selected and self.othEvPix:
                res.append(MultiContentEntryPixmapAlphaTest(pos=(r2.x + self.eventBorderWidth, r2.y + self.eventBorderWidth), size=(r2.w - 2 * self.eventBorderWidth, r2.h - 2 * self.eventBorderWidth), png=self.othEvPix, flags=BT_SCALE))
            elif selected and self.selEvPix:
                res.append(MultiContentEntryPixmapAlphaTest(pos=(r2.x + self.eventBorderWidth, r2.y + self.eventBorderWidth), size=(r2.w - 2 * self.eventBorderWidth, r2.h - 2 * self.eventBorderWidth), png=self.selEvPix, flags=BT_SCALE))
        else:
            res.append(MultiContentEntryText(pos=(left, top), size=(width, height), font=1, flags=RT_HALIGN_LEFT | RT_VALIGN_CENTER, text='', color=None, color_sel=None, backcolor=self.backColor, backcolor_sel=self.backColorSelected, border_width=self.eventBorderWidth, border_color=self.borderColor))
        if events:
            ...
            for ev in events:
                ...
                elif self.eventNameWrap.lower() == 'yes':
                ...
                if bgpng is not None and self.graphic:
                    backColor = None
                    backColorSel = None
                    res.append(MultiContentEntryPixmapAlphaTest(pos=(left + xpos + self.eventBorderWidth, top + self.eventBorderWidth), size=(ewidth - 2 * self.eventBorderWidth, height - 2 * self.eventBorderWidth), png=bgpng, flags=BT_SCALE))
                else:
                    res.append(MultiContentEntryText(pos=(left + xpos, top), size=(ewidth, height), font=1, flags=RT_HALIGN_LEFT | RT_VALIGN_CENTER, text='', color=None, color_sel=None, backcolor=backColor, backcolor_sel=backColorSel, border_width=self.eventBorderWidth, border_color=self.borderColor))
                ...
                if evW < infowidth and infoPix is not None:
                    res.append(MultiContentEntryPixmapAlphaBlend(pos=(evX, evY), size=(evW, evH), png=infoPix))
                else:
                    res.append(MultiContentEntryText(pos=(evX, evY), size=(evW, evH), font=1, flags=alignnment, text=ev[1], color=foreColor, color_sel=foreColorSel, backcolor=backColor, backcolor_sel=backColorSel))
                if borderTopPix is not None and self.graphic:
                    res.append(MultiContentEntryPixmapAlphaTest(pos=(left + xpos, top), size=(ewidth, self.eventBorderWidth), png=borderTopPix, flags=BT_SCALE))
                if borderBottomPix is not None and self.graphic:
                    res.append(MultiContentEntryPixmapAlphaTest(pos=(left + xpos, height - self.eventBorderWidth), size=(ewidth, self.eventBorderWidth), png=borderBottomPix, flags=BT_SCALE))
                if borderLeftPix is not None and self.graphic:
                    res.append(MultiContentEntryPixmapAlphaTest(pos=(left + xpos, top), size=(self.eventBorderWidth, height), png=borderLeftPix, flags=BT_SCALE))
                if borderRightPix is not None and self.graphic:
                    res.append(MultiContentEntryPixmapAlphaTest(pos=(left + xpos + ewidth - self.eventBorderWidth, top), size=(self.eventBorderWidth, height), png=borderRightPix, flags=BT_SCALE))
                if clock_types is not None and ewidth > 23:
                    ...
                    if self.screenwidth and self.screenwidth == 1920:
                        res.append(MultiContentEntryPixmapAlphaBlend(pos=pos, size=(25, 25), png=clocks))
                    else:
                        res.append(MultiContentEntryPixmapAlphaBlend(pos=pos, size=(21, 21), png=clocks))
                    if self.wasEntryAutoTimer and clock_types in (2, 7, 12):
                        if self.screenwidth and self.screenwidth == 1920:
                            res.append(MultiContentEntryPixmapAlphaBlend(pos=(pos[0] - 29, pos[1]), size=(25, 25), png=self.autotimericon))
                        else:
                            res.append(MultiContentEntryPixmapAlphaBlend(pos=(pos[0] - 22, pos[1]), size=(21, 21), png=self.autotimericon))
    ...
    def fillSimilarList(self, refstr, event_id):
        ...
        if event_id is None:
            ...
            self.list = self.epgcache.search(('RIBND',
             1024,
        ...
        if event_rect is None or time_epoch is None or time_base is None:
            ...
            if self.time_base != time_base or self.time_epoch != time_epoch or force:
                ...
                if bgpng is not None and self.graphic:
                    backColor = None
                    backColorSel = None
                    res.append(MultiContentEntryPixmapAlphaTest(pos=(0, 0), size=(service_rect.width(), self.listHeight), png=bgpng, flags=BT_SCALE))
                else:
                    res.append(MultiContentEntryText(pos=(0, 0), size=(service_rect.width(), self.listHeight), color=foreColor, backcolor=backColor, border_width=self.borderWidth, border_color=self.borderColor))
                res.append(MultiContentEntryText(pos=(5, 0), size=(service_rect.width() - 15, self.listHeight), font=0, flags=alignnment, text=_(datestr), color=foreColor, backcolor=backColor))
                bgpng = self.TlTime
                xpos = 0
                if bgpng is not None and self.graphic:
                    backColor = None
                    backColorSel = None
                    res.append(MultiContentEntryPixmapAlphaTest(pos=(service_rect.width(), 0), size=(event_rect.width(), self.listHeight), png=bgpng, flags=BT_SCALE))
                else:
                    res.append(MultiContentEntryText(pos=(service_rect.width(), 0), size=(event_rect.width(), self.listHeight), color=foreColor, backcolor=backColor, border_width=self.borderWidth, border_color=self.borderColor))
                for x in range(0, num_lines):
                    ...
                    elif int(strftime('%H', ttime)) > 12:
                        timetext = strftime('%-I:%M', ttime) + _('pm')
                    else:
                        timetext = strftime('%-I:%M', ttime) + _('am')
                    res.append(MultiContentEntryText(pos=(service_rect.width() + xpos, 0), size=(incWidth, self.listHeight), font=0, flags=RT_HALIGN_LEFT | RT_VALIGN_TOP, text=timetext, color=foreColor, backcolor=backColor))
                    line = time_lines[x]
    ...
    def buildEntry(self, name, func):
        ...
        if self.bouquetNameAlign.lower() == 'left':
            if self.bouquetNameWrap.lower() == 'yes':
                ...
                alignnment = RT_HALIGN_LEFT | RT_VALIGN_CENTER
        elif self.bouquetNameWrap.lower() == 'yes':
            alignnment = RT_HALIGN_CENTER | RT_VALIGN_CENTER | RT_WRAP
        else:
            alignnment = RT_HALIGN_CENTER | RT_VALIGN_CENTER
        res = [None]
        if selected:
            ...
            if bgpng is not None and self.graphic:
                ...
                backColorSel = None
        if bgpng is not None and self.graphic:
            res.append(MultiContentEntryPixmapAlphaTest(pos=(left + self.BorderWidth, top + self.BorderWidth), size=(width - 2 * self.BorderWidth, height - 2 * self.BorderWidth), png=bgpng, flags=BT_SCALE))
        else:
            res.append(MultiContentEntryText(pos=(left, top), size=(width, height), font=0, flags=RT_HALIGN_LEFT | RT_VALIGN_CENTER, text='', color=None, color_sel=None, backcolor=backColor, backcolor_sel=backColorSel, border_width=self.BorderWidth, border_color=self.borderColor))
        ...
        res.append(MultiContentEntryText(pos=(evX, evY), size=(evW, evH), font=0, flags=alignnment, text=name, color=foreColor, color_sel=foreColorSel, backcolor=backColor, backcolor_sel=backColorSel))
        if self.graphic:
            if borderTopPix is not None:
                res.append(MultiContentEntryPixmapAlphaTest(pos=(left, r1.y), size=(r1.w, self.BorderWidth), png=borderTopPix, flags=BT_SCALE))
            if borderBottomPix is not None:
                res.append(MultiContentEntryPixmapAlphaTest(pos=(left, r1.h - self.BorderWidth), size=(r1.w, self.BorderWidth), png=borderBottomPix, flags=BT_SCALE))
            if borderLeftPix is not None:
                res.append(MultiContentEntryPixmapAlphaTest(pos=(left, r1.y), size=(self.BorderWidth, r1.h), png=borderLeftPix, flags=BT_SCALE))
            if borderRightPix is not None:
                res.append(MultiContentEntryPixmapAlphaTest(pos=(r1.w - self.BorderWidth, left), size=(self.BorderWidth, r1.h), png=borderRightPix, flags=BT_SCALE))
        return res
