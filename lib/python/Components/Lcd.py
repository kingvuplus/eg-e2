# Embedded file name: /usr/lib/enigma2/python/Components/Lcd.py
...
def InitLcd():
    ...
    if detected:
        if can_lcdmodechecking:
            ...
            config.lcd.minitvpipmode = ConfigSelection(choices={'0': _('off'),
             '5': _('PIP'),
             '7': _('PIP with OSD')}, default='0')
            if config.misc.boxtype.value == 'gbquad' or config.misc.boxtype.value == 'gbquadplus':
                config.lcd.minitvpipmode.addNotifier(setLCDModePiP)
            else:
                config.lcd.minitvpipmode = ConfigNothing()
            config.lcd.minitvmode = ConfigSelection(choices={'0': _('normal'),
             '1': _('MiniTV'),
             '2': _('OSD'),
             '3': _('MiniTV with OSD')}, default='0')
            config.lcd.minitvfps = ConfigSlider(default=30, limits=(0, 30))
            config.lcd.minitvmode.addNotifier(setLCDModeMinitTV)
            config.lcd.minitvfps.addNotifier(setMiniTVFPS)
        else:
