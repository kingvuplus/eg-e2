# Embedded file name: /usr/lib/enigma2/python/Components/NimManager.py
from time import localtime, mktime
from datetime import datetime
import xml.etree.cElementTree
from os import path
from enigma import eDVBSatelliteEquipmentControl as secClass, eDVBSatelliteLNBParameters as lnbParam, eDVBSatelliteDiseqcParameters as diseqcParam, eDVBSatelliteSwitchParameters as switchParam, eDVBSatelliteRotorParameters as rotorParam, eDVBResourceManager, eDVBDB, eEnv, iDVBFrontend
        ...
        for slot in nim_slots:
            if slot.type is not None:
                used_nim_slots.append((slot.slot,
                 slot.description,
                 slot.config.configMode.value != 'nothing' and True or False,
                 slot.isCompatible('DVB-S2'),
                    ...
                    if nim.configMode.value == 'equal':
                        ...
                        if nim.diseqcMode.value == 'single':
                            ...
                            if nim.simpleSingleSendDiSEqC.value:
                                self.addLNBSimple(sec, slotid=x, orbpos=nim.diseqcA.orbital_position, toneburstmode=diseqcParam.NO, diseqcmode=diseqcParam.V1_0, setVoltageTone=setVoltageTone, diseqcpos=diseqcParam.AA, diseqc13V=nim.diseqc13V.value, CircularLNB=currentCircular)
                            else:
                                self.addLNBSimple(sec, slotid=x, orbpos=nim.diseqcA.orbital_position, toneburstmode=diseqcParam.NO, diseqcmode=diseqcParam.NONE, setVoltageTone=setVoltageTone, diseqcpos=diseqcParam.SENDNO, diseqc13V=nim.diseqc13V.value, CircularLNB=currentCircular)
                ...
                if currLnb.lof.value == 'universal_lnb':
                    ...
                    def setupUnicable(configManufacturer, ProductDict):
                        ...
                        if product_name in manufacturer_scr:
                            ...
                            if diction != 'EN50607' or posnum <= manufacturer_positions_value and x <= maxFixedLnbPositions:
                                if diction == 'EN50607':
...
class NimManager():
    ...
    def enumerateNIMs(self):
        ...
        for id, entry in entries.items():
            ...
            if entry.has_key('frontend_device'):
...
def InitNimManager(nimmgr, update_slots = []):
    ...
    advanced_lnb_satcr_user_choicesEN50494 = [('1', 'SatCR 1'),
     ('2', 'SatCR 2'),
     ('3', 'SatCR 3'),
     ('4', 'SatCR 4'),
     ('5', 'SatCR 5'),
     ...
     ('7', 'SatCR 7'),
     ('8', 'SatCR 8')]
    advanced_lnb_satcr_user_choicesEN50607 = [('1', 'SatCR 1'),
     ('2', 'SatCR 2'),
     ('3', 'SatCR 3'),
     ('4', 'SatCR 4'),
     ('5', 'SatCR 5'),
    ...
    for slot in nimmgr.nim_slots:
        ...
        print '[NimManager] slotname = %s, slotdescription = %s, multitype = %s, current type = %s' % (slot.input_name,
         slot.description,
         slot.isMultiType() and addMultiType,
         slot.getType())
