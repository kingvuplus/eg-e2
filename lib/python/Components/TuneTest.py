# Embedded file name: /usr/lib/enigma2/python/Components/TuneTest.py
...
class Tuner:
    ...
    def tuneTerr(self, frequency, inversion = 2, bandwidth = 7000000, fechigh = 6, feclow = 6, modulation = 2, transmission = 2, guard = 4, hierarchy = 4, system = 0, plpid = 0):
        if self.frontend:
                        ...
                        if self.tsid != self.currTuned[10] or self.onid != self.currTuned[11]:
                            self.failedTune.append([self.currTuned,
                             ...
                             {'real': (self.tsid, self.onid),
                              'expected': (self.currTuned[10], self.currTuned[11])},
                             dict])
                            pidsFailed = True
                        else:
