# Embedded file name: /usr/lib/enigma2/python/Screens/OScamInfo.py
...
class OscamInfo():
    ...
    def confPath(self):
        fp = open('/tmp/.oscam/oscam.version', 'r')
        oscamVer = fp.readlines()
        fp.close()
        info = {}
        for line in oscamVer:
            if 'ConfigDir:' in line:
                d = line.split(':', 1)
                if len(d) > 1:
                    info[d[0].strip()] = d[1].strip()

        data = info.get('ConfigDir', '') + 'oscam.conf'
        if fileExists(data):
            return data
        else:
...
class oscECMInfo(Screen, OscamInfo):
    ...
    def buildListEntry(self, listentry):
        screenwidth = getDesktop(0).size().width()
        if screenwidth and screenwidth == 1920:
            return [None, (eListboxPythonMultiContent.TYPE_TEXT,
              15,
              3,
              450,
              50,
    ...
    def __init__(self, session, what):
        ...
        if HDSKIN:
            for k, v in enumerate(['red',
             ...
             'blue']):
                xpos = k * button_width
                self.skin += '<ePixmap name="%s" position="%d,%d" size="63,43" pixmap="buttons/key_%s.png" zPosition="1" transparent="1" alphatest="on" />' % (v,
                 xpos,
                 ypos,
                 v)
            ...
            for k, v in enumerate(['red',
             ...
             'blue']):
                xpos = k * button_width
                self.skin += '<ePixmap name="%s" position="%d,%d" size="35,25" pixmap="buttons/key_%s.png" zPosition="1" transparent="1" alphatest="on" />' % (v,
                 xpos,
                 ypos,
                 v)
                self.skin += '<widget source="key_%s" render="Label" position="%d,%d" size="%d,%d" font="Regular;18" zPosition="1" valign="center" transparent="1" />' % (v,
        ...
        if config.oscaminfo.autoupdate.value:
            ...
            timeout = config.oscaminfo.intervall.value * 1000
            self.loop.start(timeout, False)
        self['actions'] = ActionMap(['OkCancelActions', 'ColorActions'], {'ok': self.showData,
         'cancel': self.exit,
         'red': self.exit,
         'green': self.key_green,
         'yellow': self.key_yellow,
         'blue': self.key_blue}, -1)
        self.onLayoutFinish.append(self.showData)
