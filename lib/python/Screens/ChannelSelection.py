...
class ChannelContextMenu(Screen):
    ...
    def setStartupServiceCallback(self, answer):
        if answer:
            ...
            if answer == 'standby':
                config.servicelist.startupservice_onstandby.value = True
    ...
    def removeSatelliteServices(self):
        ...
        if unsigned_orbpos == 65535:
            ...
            if unsigned_orbpos > 1800:
                ...
                direction = _('E')
            messageText = _('Are you sure you want to remove all %d.%d%s%s services?') % (unsigned_orbpos / 10,
             unsigned_orbpos % 10,
             '\xc2\xb0',
             direction)
...
class ChannelSelection(ChannelSelectionBase, ChannelSelectionEdit, ChannelSelectionEPG, SelectionEventInfo):
    ...
    def channelSelected(self, doClose = True):
        if config.usage.servicelistpreview_mode.value and self.getCurrentSelection() != self.session.nav.getCurrentlyPlayingServiceOrGroup():
            doClose = False
