# Embedded file name: /usr/lib/enigma2/python/Screens/ScanSetup.py
...
def buildTerTransponder(frequency, inversion = 2, bandwidth = 7000000, fechigh = 6, feclow = 6, modulation = 2, transmission = 2, guard = 4, hierarchy = 4, system = 0, plpid = 0):
...
class ScanSetup(ConfigListScreen, Screen, CableTransponderSearchSupport, TerrestrialTransponderSearchSupport):
    ...
    def createConfig(self, frontendData):
        ...
        self.scan_sat.modulation = ConfigSelection(default=defaultSat['modulation'], choices=[(eDVBFrontendParametersSatellite.Modulation_QPSK, 'QPSK'), (eDVBFrontendParametersSatellite.Modulation_8PSK, '8PSK')])
        ...
        self.scan_ter.complete_system = ConfigSelection(default=eDVBFrontendParametersTerrestrial.System_DVB_T_T2, choices=[(eDVBFrontendParametersTerrestrial.System_DVB_T_T2, _('Auto')), (eDVBFrontendParametersTerrestrial.System_DVB_T, _('DVB-T')), (eDVBFrontendParametersTerrestrial.System_DVB_T2, _('DVB-T2'))])
            ...
            if nim.isCompatible('DVB-S'):
                ...
                if self.scan_type.value == 'single_transponder':
                    ...
                    if len(nimsats):
                        ...
                        self.addSatTransponder(tlist, self.scan_sat.frequency.value, self.scan_sat.symbolrate.value, self.scan_sat.polarization.value, fec, self.scan_sat.inversion.value, orbpos, self.scan_sat.system.value, self.scan_sat.modulation.value, self.scan_sat.rolloff.value, self.scan_sat.pilot.value)
                    removeAll = False
...
global cable_autoscan_nimtype ## Warning: Unused global