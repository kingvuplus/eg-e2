# Embedded file name: /usr/lib/enigma2/python/Screens/LogManager.py
...
class LogManagerPoller():
    ...
    def JobTrim(self):
        ...
        self.TrimTimer.startLongTimer(3600)
...
class LogManager(Screen):
    ...
    def __init__(self, session):
        ...
        self['myactions'] = ActionMap(['ColorActions', 'OkCancelActions', 'DirectionActions'], {'ok': self.changeSelectionState,
         'cancel': self.close,
    ...
    def doSendlog(self, additonalinfo = None):
        ...
        if config.logmanager.user.value != '' and config.logmanager.useremail.value != '':
            ...
            toegamilogs = 'info@egami-image.com'
            msg['From'] = fromlogman
            msg['To'] = toegamilogs
            msg['Cc'] = fromlogman
            ...
            if self.sendallfiles:
                ...
                self.saveSelection()
            wos_user = 'egamilogs@egami-image.com'
            wos_pwd = base64.b64decode('xxxxxxxxxxxxxxx')
            try:
                print '[LogManager] connecting to server: mail.oe-alliance.com'
                s = smtplib.SMTP('mail.egami.com', 26)
                s.login(wos_user, wos_pwd)
                if config.logmanager.usersendcopy.value:
                    s.sendmail(fromlogman, [toegamilogs, fromlogman], msg.as_string())
                    s.quit()
                    self.session.open(MessageBox, sentfiles + ' ' + _('has been sent to the EGAMI beta team.\nplease quote') + ' ' + str(ref) + ' ' + _('when asking question about this log\n\nA copy has been sent to yourself.'), MessageBox.TYPE_INFO)
                else:
                    s.sendmail(fromlogman, toegamilogs, msg.as_string())
                    s.quit()
...
class LogManagerViewLog(Screen):
    ...
    def __init__(self, session, selected):
        ...
        self['setupActions'] = ActionMap(['SetupActions', 'ColorActions', 'DirectionActions'], {'cancel': self.cancel,
         'ok': self.cancel,
         'up': self['list'].pageUp,
         'down': self['list'].pageDown,
         'right': self['list'].lastPage}, -2)
