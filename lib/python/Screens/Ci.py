# Embedded file name: /usr/lib/enigma2/python/Screens/Ci.py
...
def InitCiConfig():
    ...
    for slot in range(MAX_NUM_CI):
        ...
        if SystemInfo['CommonInterfaceSupportsHighBitrates']:
            if getBrandOEM() in ('dags', 'blackbox', 'ini', 'skylake', 'vuplus'):
...
class CiSelection(Screen):
    ...
    def appendEntries(self, slot, state):
        ...
        self.list.append((_('Enter persistent PIN code'),
         ConfigNothing(),
         5,
         slot))
        self.list.append((_('Reset persistent PIN code'),
         ConfigNothing(),
         6,
         slot))
